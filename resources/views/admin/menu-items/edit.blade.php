...
@section('content')
    ...
    <div class="bg-white border border-zinc-200 shadow-sm overflow-hidden">
        <div class="border-b border-zinc-200 px-8 py-6 flex justify-between items-center bg-zinc-50/50">
            <div>
                <h3 class="font-serif text-2xl text-zinc-900 tracking-tight">Perbarui Data Produk</h3>
                <p class="text-xs text-zinc-500 mt-1">Ubah detail item menu yang sudah terdaftar di pangkalan data.</p>
            </div>
            <i class="ph ph-pencil-simple text-3xl text-zinc-300"></i>
        </div>

        <form action="{{ route('menu-items.update', $menuItem->id) }}" method="POST" class="p-8 group">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8 mb-8">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" class="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 focus:bg-white focus:outline-none transition-colors duration-300 text-sm" placeholder="Jelaskan komposisi atau detail produk di sini...">{{ old('description', $menuItem->description) }}</textarea>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-8 border-t border-zinc-200 mt-4">
                ...
                <button type="submit" class="bg-zinc-900 text-white px-8 py-3 text-xs font-bold uppercase tracking-widest hover:bg-zinc-800 transition-colors duration-300 flex items-center gap-2 shadow-md group-invalid:bg-zinc-300 group-invalid:text-zinc-500 group-invalid:cursor-not-allowed group-invalid:shadow-none">
                    Perbarui Data <i class="ph ph-check text-base"></i>
                </button>
            </div>
        </form>
    </div>
