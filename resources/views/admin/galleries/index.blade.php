...
@section('content')
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 border-b border-zinc-200 pb-6">
        <div>
            <p class="text-xs font-bold uppercase tracking-[0.2em] text-zinc-400 mb-2">Direktori Visual</p>
            <h2 class="text-3xl font-serif text-zinc-900 tracking-tight">Galeri Dokumentasi</h2>
            <p class="text-sm text-zinc-500 mt-2">Kelola seluruh aset visual perusahaan yang dipublikasikan pada halaman utama situs web.</p>
        </div>
        ...
        <div>
            <a href="{{ route('galleries.create') }}" class="inline-flex items-center gap-2 bg-zinc-900 text-white px-6 py-3 text-xs font-bold uppercase tracking-widest hover:bg-zinc-800 transition-colors duration-300 shadow-sm">
                <i class="ph ph-upload-simple text-base"></i> Unggah Aset Baru
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div class="col-span-full py-20 bg-zinc-50 border border-zinc-200 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <h3 class="text-xl font-serif text-zinc-900 mb-2">Galeri Masih Kosong</h3>
            <p class="text-sm text-zinc-500 max-w-sm leading-relaxed">Sistem belum memiliki aset visual. Silakan unggah dokumentasi baru melalui tombol di kanan atas.</p>
        </div>
        @endforelse
    </div>
@endsection