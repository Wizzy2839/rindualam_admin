...
@section('header', 'Manajemen Aset Visual')
...
@section('content')
    <div class="mb-8">
        <a href="{{ route('galleries.index') }}" class="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-zinc-900 transition-colors duration-300">
            <i class="ph ph-arrow-left text-base"></i> Kembali ke Direktori Galeri
        </a>
    </div>
    ...
    <div class="bg-white border border-zinc-200 shadow-sm overflow-hidden">
        <div class="border-b border-zinc-200 px-8 py-6 bg-zinc-50/50">
            <h3 class="font-serif text-2xl text-zinc-900 tracking-tight">Unggah Dokumentasi (Massal)</h3>
            <p class="text-xs text-zinc-500 mt-1">Pilih atau seret dan lepas beberapa berkas foto sekaligus untuk dipublikasikan pada galeri utama.</p>
        </div>
        ...
        <form action="{{ route('galleries.store') }}" method="POST" enctype="multipart/form-data" class="p-8 group">
            ...
            <div class="mb-8">
                <label class="block text-xs font-bold tracking-widest text-zinc-500 uppercase mb-3">Pilih Berkas Foto <span class="text-red-500">*</span></label>
            ...
            <div class="mb-8 bg-zinc-50 border border-zinc-200 p-6 flex flex-col gap-4">
                <div>
                    <label class="block text-xs font-bold tracking-widest text-zinc-500 uppercase mb-2">Keterangan Massal (Opsional)</label>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <p class="text-xs text-zinc-600 leading-relaxed">Catatan: Teks keterangan yang Anda masukkan di sini akan diterapkan pada seluruh foto yang diunggah dalam sesi ini. Untuk keterangan yang berbeda, harap unggah foto secara terpisah.</p>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-8 border-t border-zinc-200">
                ...
                <button type="submit" class="bg-zinc-900 text-white px-8 py-3 text-xs font-bold uppercase tracking-widest hover:bg-zinc-800 transition-colors duration-300 flex items-center gap-2 group-invalid:bg-zinc-300 group-invalid:text-zinc-500 group-invalid:cursor-not-allowed">
                    Unggah Semua Foto <i class="ph ph-cloud-arrow-up text-base"></i>
                </button>
            </div>
        </form>
    </div>
